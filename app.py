...
# --- Константы ---
OUTPUT_DIR = '/content/drive/MyDrive/SpeechViz3D'
AUDIO_PATH = "/content/drive/MyDrive/SpeechViz3D/input_audio/charon_converted.wav"
...
def transcribe_audio_with_whisper(audio_path, model_size="medium"):
    ...
    print(f"Транскрибация завершена. Распознанный текст: {full_text}")
...
def analyze_vowel_segments(audio_path, transcription_segments):
    ...
    try:
        ...
        print(f"КРИТИЧЕСКАЯ ОШИБКА: Не удалось обработать аудиофайл: {e}")
        return []
    if not transcription_segments:
        print("Список транскрибированных сегментов пуст.")
        return []
    for segment in transcription_segments:
        ...
        for phoneme in phonemes_in_word:
            ...
            if not (math.isnan(median_f1) or math.isnan(median_f2) or math.isnan(duration) or math.isnan(median_pitch)):
                ...
                print(f"Фонема '{phoneme}' в слове '{word}': F1={median_f1:.2f}, F2={median_f2:.2f}, Длит-сть={duration:.2f}, Тон={median_pitch:.2f} Гц, Интенсивность={median_intensity:.2f} дБ, Энергия={total_energy:.6f} Pa^2·sec")
            current_time = vowel_segment_end
    print(f"Всего данных о гласных собрано: {len(vowel_data)}")
    return vowel_data

def plot_phoneme_frequency(vowel_data, audio_filename):
    """Строит столбчатую диаграмму частоты фонем."""
    base_name = os.path.splitext(os.path.basename(audio_filename))[0]
    if not vowel_data:
        print("Нет данных для построения графика частоты фонем.")
        return
    df = pd.DataFrame(vowel_data)
    phoneme_counts = df['vowel'].value_counts()
    vowel_colors = {'и': 'blue', 'э': 'green', 'а': 'yellow', 'о': 'orange', 'у': 'purple', 'ы': 'pink'}

    fig = go.Figure()
    fig.add_trace(go.Bar(
        x=phoneme_counts.index,
        y=phoneme_counts.values,
        marker_color=[vowel_colors.get(vowel, 'gray') for vowel in phoneme_counts.index],
        text=phoneme_counts.values,
        textposition='auto',
        hoverinfo='text',
        hovertext=[f'Фонема: {vowel}<br>Количество: {count}' for vowel, count in phoneme_counts.items()]
    ))

    fig.update_layout(
        title=f'Частота гласных фонем - {base_name}',
        xaxis_title='Фонема',
    ...
    html_path = os.path.join(OUTPUT_DIR, f"{base_name}_phoneme_frequency.html")
...
def plot_3d_with_polygons(vowel_data, audio_filename):
    ...
    соединенными по часовой стрелке, и градиентом энергии.
    """
    base_name = os.path.splitext(os.path.basename(audio_filename))[0]
    if not vowel_data:
    ...
    for vowel, group in df.groupby('vowel'):
        ...
        print(f"Фонема '{vowel}' (высшая точка по длительности):")
        print(f"  Тон (mean_pitch): {highest_point['mean_pitch']:.2f} Гц")
        print(f"  Логарифм тона (log_pitch): {highest_point['log_pitch']:.4f}")
        print(f"  Нормализованный логарифм тона (norm_log_pitch): {highest_point['norm_log_pitch']:.4f}")
        print(f"  Энергия (total_energy): {highest_point['total_energy']:.6f} Pa^2·sec")
        print(f"  Нормализованная энергия (norm_energy): {highest_point['norm_energy']:.4f}")
            ...
            try:
                ...
                if len(below_x) >= 3:
                    ...
                    try:
                        ...
                        print(f"Не удалось построить область ниже плоскости для фонемы '{vowel}': {e}")
                ...
                print(f"Не удалось построить многоугольник для фонемы '{vowel}': {e}")
...
if transcription_segments:
    ...
    if vowel_data:
        ...
        html_path_3d = plot_3d_with_polygons(vowel_data, AUDIO_PATH)
        print(f"\nИнтерактивный 3D-график с многоугольниками и срезами сохранен в: {html_path_3d}")
        
        # Построение графика частоты фонем
        html_path_freq = plot_phoneme_frequency(vowel_data, AUDIO_PATH)
        print(f"\nИнтерактивный график частоты фонем сохранен в: {html_path_freq}")
    else:
